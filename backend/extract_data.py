...
from config import DOWNLOADS_FOLDER, EXTRACTED_FOLDER
...
def save_combined_data(folder_path, combined_data):
    """
    Save the combined data (PDF content + metadata) as a JSON file.

    Args:
        folder_path (str): Path to the folder where JSON file will be saved.
        combined_data (dict): Combined data to be saved.
    """
    output_path = os.path.join(folder_path, "combined_data.json")
    ...
    for folder in os.listdir(EXTRACTED_FOLDER):
        ...
        if os.path.isdir(folder_path):
            try:
                ...
                save_combined_data(folder_path, combined_data)
